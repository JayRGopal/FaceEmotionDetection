...
for metric in METRICS:
    ...
    for patient in patients:
        try:
            ...
            # Load predictions and calculate Pearson's R
            predictions_prefix_1 = load_var(f'predictions_{patient}_{PREFIX_1 if metric != "Pain" else PREFIX_1_PAIN}', RUNTIME_VAR_PATH)[f'{metric}']
            r_1 = np.corrcoef(predictions_prefix_1['y_true'], predictions_prefix_1['preds'][predictions_prefix_1['best_time_radius']])[0, 1]

            if not np.isnan(r_1):
                r_values_included.append(r_1)
                random_distributions.append(generate_random_chance_distribution(predictions_prefix_1['y_true'], predictions_prefix_1['preds'][predictions_prefix_1['best_time_radius']]))
    ...
    def plot_results(r_values, group_label):
        data = [r_values]
        labels = [LABEL_1]
        ...
        plt.title(f'Predicting {metric.capitalize()} - {group_label}')
        plt.xlabel("Pearson's R")
        plt.savefig(os.path.join(RESULTS_PATH_BASE, f'{metric}_{group_label}_groupR.png'), bbox_inches='tight')
        plt.close()

    # Plot and save results for included patients
    plot_results(r_values_included, 'Included')

    # Optionally, plot and save results for excluded patients (if there are any)
    if patients_excluded:
        r_values_excluded = [np.nan for _ in patients_excluded]  # Set empty for excluded patients
        plot_results(r_values_excluded, 'Excluded')

print("Separate plots generated for patients who meet and do not meet inclusion criteria.")
