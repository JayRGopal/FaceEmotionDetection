...
def save_pearsons_r2(predictions_dict, output_folder):
    ...
    for metric, data in predictions_dict.items():
        best_time_radius = data['best_time_radius']
        y_true = data['y_true']
        y_pred = data['preds'][best_time_radius]
        
        # Calculate R² (square of Pearson's R)
        pearson_corr, _ = pearsonr(y_true, y_pred)
        r2 = pearson_corr ** 2
        r2_results.append({'Metric': metric, 'Time Radius (minutes)': best_time_radius, 'R²': r2})
    
    # Save R² results to CSV
    r2_df = pd.DataFrame(r2_results)
    r2_output_path = os.path.join(output_folder, "pearsons_r2_results.csv")
    r2_df.to_csv(r2_output_path, index=False)
