...
for RESULTS_PREFIX in RESULTS_PREFIX_LIST:
    ...
    for metric in all_metrics:
        ...
        # We first remove low-variance features using VarianceThreshold (threshold=0.1) to discard irrelevant features.
        # Then, we apply PCA to retain 95% of the variance, reducing dimensionality and focusing subsequent LASSO regression on the most informative components.
        from sklearn.feature_selection import VarianceThreshold
        from sklearn.decomposition import PCA

        variance_threshold = 0.1   # Features with variance below this value are removed
        pca_variance_ratio = 0.95  # Retain components that explain 95% of the variance

        for key in vectors_return.keys():
            selector = VarianceThreshold(threshold=variance_threshold)
            selected_features = selector.fit_transform(vectors_return[key])
            pca = PCA(n_components=pca_variance_ratio)
            vectors_return[key] = pca.fit_transform(selected_features)
        # ------------------------------------
