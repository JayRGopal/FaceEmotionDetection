...
for i in all_videos:
  ...
  if os.path.exists(save_file):
    ...
    if JUST_AU:
      cmd = f'/home/klab/Desktop/OpenFace/build/bin/FaceLandmarkVidMulti -f "{video_path}" -out_dir "{OUTPUT_DIRECTORY}" -aus'
    else:
      cmd = f'/home/klab/Desktop/OpenFace/build/bin/FaceLandmarkVidMulti -f "{video_path}" -out_dir "{OUTPUT_DIRECTORY}" -aus -2Dfp -3Dfp -pdmparams -pose -gaze'
    if AU_STATIC:
      cmd = cmd + ' -au_static'
